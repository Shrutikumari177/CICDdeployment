    ...
    return Controller.extend("com.ingenx.nauti.masterdashboard.controller.CurrencyType", {
      ...
      onSave: function () {
        ...
        oTable.getItems().forEach(function (row) {
          var value1 = row.getCells()[0].getValue().toUpperCase();
          var value2 = row.getCells()[1].getValue();
          var value3 = row.getCells()[2].getValue();

          // Check for duplicate codes in the table
          if (tempCodesArray.includes(value1)) {
          ...
          if (!value1 || !value2 || !value3) {
            errors.push("Please enter all fields for each row.");
          } else {
            validEntries.push({
              Navoycur: value1,
              Navoygcurdes: that.formatDes(value2),
              Navoycountry: value3
            });
          }
        });

        // Step 2: Check for duplicates in the backend and validate further
        ...
        oBindListSP.attachEventOnce("dataReceived", function () {
          ...
          // Check for duplicates with existing entries in the backend
          validEntries.forEach(function (entry) {
            if (existingEntries.includes(entry.Navoycur)) {
              duplicateEntries.push(entry.Navoycur);
            }
          });

          // Step 3: Handle validation results
          if (errors.length === 0 && duplicateEntries.length === 0) {
            // If no errors or duplicates, proceed to post data
            createEntries();
          } else {
        ...
        oBindListSP.getContexts(); // Trigger data retrieval

        // Step 4: Post valid entries to S/4
        function createEntries() {
          validEntries.forEach(function (entry) {
            var oBindListSP = that.getView().getModel().bindList("/CurTypeSet");

            try {
              oBindListSP.create(entry);
            } catch (error) {
              sap.m.MessageToast.show("Error while saving data");
            }
          });

          // Refresh the model and reset the view after successful posting
          that.getView().getModel().refresh();
          that.resetView();
