    ...
    function (Controller,DateFormat,  JSONModel, MessageToast) {
        ...
        return Controller.extend("com.ingenx.nauti.biddingcontroller.controller.Bidding", {
            ...
            _onObjectMatched2: async function(oEvent) {
                ...
                const bStatus = navModel.getProperty("/navigatedstatus");
                
                let newStartDate = this.dateFormat(BidStartDate)
                let newEndDate = this.dateFormat(BidEndDate)
                try {
                    this._startTimer(newStartDate,BidStartTime,newEndDate,BidEndTime,bStatus)
            ...
            _sendControllerDataToS4: function(oPayload) {
            ...
            createEntries: async function (payLoad) {
                ...
                let oBindList = oModel.bindList("/ControllerLiveBidDetails");
                let res = await oBindList.create(payLoad,true);
                oModel.refresh()
                return res;
            },
            ...
            // This method is used for starting the bidding value on table
            ...
            nStartBidding: async function() {
                ...
                try {
                    ...
                    let oModel = this.getView().getModel("rankmodel");
                    if (!oModel) {
                        console.error("rankmodel is not defined.");
                        return;
                    }
                    let aVendors = oModel.getProperty("/vendors");
            ...
            // This method used for started the timer when bidding started
            _startTimer: function(startDate, startTime, endDate, endTime,status) {
                if (this.intervalId) {
                ...
                oTimeText.setText("");
                let startBtn = this.byId("startButton")
                let bidLabel = this.byId("_IDGenLabel7")
                let submitBox = this.byId("submitBox")
               var that = this
                if (status === "CLOS") {
                    startBtn.setVisible(false)
                    bidLabel.setText("Last Bid")
                    ...
                    if (remainingTime <= 600) { 
                        ...
                        if (remainingTime <= 0) {
                            ...
                            sap.m.MessageBox.show("Bidding successfully completed.", {
                                ...
                                actions: [sap.m.MessageBox.Action.OK],
                                onClose: function (oAction) {
                                    that.triggerControllerPostLineItemData()
